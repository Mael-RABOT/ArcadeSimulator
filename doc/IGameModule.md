...
class IGameModule {
    public:
        ...
        virtual void handleInput(std::size_t deltaTime, Input input, std::unordered_map<std::size_t, std::reference_wrapper<Entity>> entities) = 0;
        virtual void update(std::size_t deltaTime, std::vector<std::reference_wrapper<Entity>> entities) = 0; //delta since the start of the game
        virtual std::vector<std::pair<std::string, Vector2D>> getTexts() = 0;
        virtual std::vector<std::reference_wrapper<Entity>> initEntities(Map &map) = 0;
};
